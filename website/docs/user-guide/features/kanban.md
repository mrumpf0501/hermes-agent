...
| `kanban.default_workdir` | unset | Board-level project directory (`boards set-default-workdir`). New tasks with no explicit workspace use `dir` + this path; omit workspace in API/UI/tools to inherit it. Per-task `workspace:` still wins. |
